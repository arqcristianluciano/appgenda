  ...
  if (!loaded) return (
    <div className="h-screen flex items-center justify-center bg-[#F7F4EF]">
      <div className="text-center">
        <div className="text-3xl font-extrabold text-[#1C1A17] tracking-tight mb-2">CLS</div>
        <div className="text-xs text-gray-400">Cargando agenda…</div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F7F4EF]">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Mobile top bar */}
        <div className="lg:hidden flex items-center justify-between px-4 bg-[#1C1A17] flex-shrink-0" style={{height:52}}>
        ...
        <div className="hidden lg:flex items-center justify-between px-8 pt-7 pb-0 flex-shrink-0">
          <div>
            <h1 className="text-2xl font-extrabold text-[#1C1A17] tracking-tight">{VIEW_TITLES[vista]}</h1>
            <p className="text-xs text-gray-400 mt-0.5">{DIAS[now.getDay()]}, {now.getDate()} de {MESES[now.getMonth()]}</p>
          </div>
        </div>
        {/* Content */}
        ...
        {/* Mobile bottom nav */}
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#1C1A17] border-t border-white/[0.08] z-50 flex" style={{height:56}}>
          {MOB_NAV.map(v => (
            <button key={v.id} onClick={() => setVista(v.id)}
              className={`flex-1 flex flex-col items-center justify-center gap-0.5 transition-colors ${vista === v.id ? 'text-[#2B5E3E]' : 'text-white/35 hover:text-white/70'}`}>
              {v.icon}
