...
const CAT_CSS: Record<CatInversion, string> = {
  inmobiliario: 'bg-[#E8F2EC] text-[#2B5E3E]',
  vehiculos: 'bg-blue-50 text-blue-700',
  financiero: 'bg-amber-50 text-amber-700',
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div className="bg-white border border-black/[0.08] rounded-xl px-5 py-4 shadow-sm">
          <div className="text-3xl font-extrabold tracking-tight">{data.inversiones.length}</div>
          <div className="text-[11px] text-gray-400 mt-1 font-medium">Activos</div>
        </div>
        <div className="bg-white border border-black/[0.08] rounded-xl px-5 py-4 shadow-sm">
          <div className="text-xl font-extrabold tracking-tight leading-none">US${Math.round(totalCompra).toLocaleString()}</div>
          <div className="text-[11px] text-gray-400 mt-1 font-medium">Invertido (aprox.)</div>
        </div>
        <div className="bg-white border border-black/[0.08] rounded-xl px-5 py-4 shadow-sm">
          <div className="text-xl font-extrabold tracking-tight leading-none">US${Math.round(totalActual).toLocaleString()}</div>
          <div className="text-[11px] text-gray-400 mt-1 font-medium">Valor actual (aprox.)</div>
        </div>
        <div className="bg-white border border-black/[0.08] rounded-xl px-5 py-4 shadow-sm">
          <div className={`text-xl font-extrabold tracking-tight leading-none ${ganancia >= 0 ? 'text-[#2B5E3E]' : 'text-red-600'}`}>
            {pctTotal ? `${ganancia >= 0 ? '+' : ''}${pctTotal}%` : '—'}
          </div>
          <div className="text-[11px] text-gray-400 mt-1 font-medium">Rentabilidad</div>
        </div>
      </div>
        ...
        {[['todas','Todas'],['inmobiliario','Inmobiliario'],['vehiculos','Vehículos'],['financiero','Financiero'],['empresas','Empresas']].map(([f,l]) => (
          <button key={f} onClick={() => setFiltroInv(f as typeof filtroInv)}
            className={`h-7 px-3 rounded-full text-[11px] font-semibold border transition-all
              ${filtroInv === f ? 'bg-[#2B5E3E] border-[#2B5E3E] text-white' : 'bg-white border-black/[0.14] text-gray-500 hover:border-[#2B5E3E] hover:text-[#2B5E3E]'}`}>
            {l}
          </button>
        ))}
        <button onClick={openAdd} className="ml-auto h-7 px-3 rounded-lg text-[11px] font-bold bg-[#2B5E3E] text-white hover:bg-[#3D7A54] transition-all">
          + Inversión
        </button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="text-[16px] font-extrabold mb-4">{editId ? 'Editar inversión' : 'Nueva inversión'}</div>
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <select className="h-9 px-3 bg-gray-50 border border-gray-200 rounded-lg text-[13px] outline-none"
              ...
              <div className="grid grid-cols-2 gap-2">
                <input type="number" className="h-9 px-3 bg-gray-50 border border-gray-200 rounded-lg text-[13px] outline-none focus:border-[#2B5E3E]"
              ...
              <input type="date" className="h-9 px-3 bg-gray-50 border border-gray-200 rounded-lg text-[13px] outline-none"
                value={form.fecha} onChange={e => setForm(f => ({ ...f, fecha: e.target.value }))} />
              <textarea className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-[13px] outline-none focus:border-[#2B5E3E] resize-none min-h-[56px]"
                placeholder="Notas…" value={form.nota} onChange={e => setForm(f => ({ ...f, nota: e.target.value }))} />
            </div>
            <div className="flex gap-2 justify-end mt-4">
              <button onClick={() => { setShowForm(false); setEditId(null) }} className="h-8 px-3 text-[12px] font-medium text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">Cancelar</button>
              <button onClick={handleSave} className="h-8 px-4 text-[12px] font-bold bg-[#2B5E3E] text-white rounded-lg hover:bg-[#3D7A54]">Guardar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-black/[0.08] rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-gray-50 text-[10px] font-bold uppercase tracking-widest text-gray-400 border-b border-black/[0.06]">
            ...
            <tbody>
              {list.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-300 text-[13px]">Sin inversiones en esta categoría</td></tr>
              ) : list.map(inv => {
                const gain = inv.compra && inv.actual ? ((inv.actual - inv.compra) / inv.compra * 100).toFixed(1) : null
                const gainCls = !gain ? 'text-gray-400' : parseFloat(gain) > 0 ? 'text-[#2B5E3E] font-bold' : 'text-red-600 font-bold'
                return (
                  <tr key={inv.id} className="border-t border-black/[0.05] hover:bg-gray-50 group">
                    <td className="px-4 py-2.5">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${CAT_CSS[inv.cat]}`}>{CAT_LABELS[inv.cat]}</span>
                    </td>
                    <td className="px-4 py-2.5 font-semibold max-w-[160px] truncate">{inv.nombre}</td>
                    <td className="px-4 py-2.5 text-gray-500">{fmtMoney(inv.compra, inv.moneda)}</td>
                    <td className="px-4 py-2.5 text-gray-500">{fmtMoney(inv.actual, inv.moneda)}</td>
                    <td className={`px-4 py-2.5 ${gainCls}`}>{gain ? `${parseFloat(gain) >= 0 ? '+' : ''}${gain}%` : '—'}</td>
                    <td className="px-4 py-2.5 text-gray-400 text-[12px]">{inv.fecha || '—'}</td>
                    <td className="px-4 py-2.5 text-gray-400 max-w-[140px] truncate text-[12px]">{inv.nota || '—'}</td>
                    <td className="px-4 py-2.5">
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(inv)} className="w-6 h-6 rounded flex items-center justify-center text-gray-300 hover:text-[#2B5E3E] hover:bg-[#E8F2EC] transition-all"><Pencil size={12} /></button>
                        <button onClick={() => { if (confirm('¿Eliminar?')) deleteInversion(inv.id) }} className="w-6 h-6 rounded flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                )
