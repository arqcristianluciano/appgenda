...
import type { AppData, Inversion, Vista, FiltroHoy, FiltroProy, FiltroInv } from '../types'
...
interface AppStore {
  ...
  updateTareaNota: (id: number, nota: string) => void
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  updateTareaNota: (id, nota) => {
    set(s => ({
      data: { ...s.data, tareas: s.data.tareas.map(t => t.id === id ? { ...t, nota } : t) }
    }))
    get().persist()
  },
